  ...
  <div v-loading="loading" class="app-composite">
    ...
    <div class="app-container">
      ...
      <div class="app-part">
        ...
        <div class="app-item">
          <el-select v-model="search.status" placeholder="请选择状态" clearable @change="initList">
            <el-option v-for="item in statusOptions" :key="item[0]" :value="item[0]" :label="item[1]"></el-option>
          </el-select>
        </div>
        <div class="app-item">
          <el-input
      ...
      <div class="app-part">
        ...
        <div class="app-item">
          <el-button type="primary" icon="el-icon-plus" @click="handleErpSync">同步ERP人员</el-button>
        </div>
      </div>
      <!-- 公共表格 -->
      <div v-loading="tableLoading" class="app-table">
        <el-table height="100%" :data="tableData" @selection-change="handleSelectionChange" row-key="id">
          ...
          <el-table-column label="员工姓名" min-width="100" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.name || '--' }}</span>
              <div>
                <el-tag v-if="scope.row.supperManager" type="warning">超级管理员</el-tag>
              </div>
            </template>
          </el-table-column>
