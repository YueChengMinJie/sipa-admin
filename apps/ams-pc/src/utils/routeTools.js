...
export function menuFilter(d2) {
  ...
  internalFun(d2, menus)
  return menus
}

export function composeRoutes(d2) {
  const routes = []
  let topLevelName = ''
  const montageRoutes = (routes, arr) => {
    arr.forEach(item => {
      if (item.type !== 'page' && item.type !== 'endpoint') {
        const attr = {
          path: item.url,
          name: item.aliasName,
          meta: {
        ...
        if (
          item.functionCodeVoList &&
          item.functionCodeVoList.length > 0 &&
          item.functionCodeVoList.filter(f => f.type !== 'page' && f.type !== 'endpoint').length > 0
        ) {
  ...
  montageRoutes(routes, d2)
  return routes
}

function getDisplayableMenuName(data) {
  if (
    data.functionCodeVoList &&
    data.functionCodeVoList.length > 0 &&
    data.functionCodeVoList.filter(f => f.type !== 'page' && f.type !== 'endpoint').length > 0
  ) {
